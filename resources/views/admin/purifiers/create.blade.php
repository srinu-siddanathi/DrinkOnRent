...
@section('title', 'Add Purifier')

@section('content')
    <div class="bg-white shadow rounded-lg">
        <div class="px-4 py-5 sm:p-6">
            <div class="flex justify-between items-center mb-4">
                <h2 class="text-lg font-medium text-gray-900">Add New Purifier</h2>
                <a href="{{ route('admin.purifiers.index') }}" class="text-indigo-600 hover:text-indigo-900">Back to List</a>
            </div>

            <form action="{{ route('admin.purifiers.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="type" id="type" 
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            <option value="ro" {{ old('type') === 'ro' ? 'selected' : '' }}>RO</option>
                            <option value="alkaline" {{ old('type') === 'alkaline' ? 'selected' : '' }}>Alkaline</option>
                        </select>
                        @error('type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
